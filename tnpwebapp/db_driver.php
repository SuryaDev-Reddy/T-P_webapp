...
function getStudentField()
{
 ...
 $q = $conn->query($sql) or die('failed');
 $r = $q->fetch(PDO::FETCH_ASSOC);
  return $r['profile_pic'];
}

function setStudentField($newname)
{
 $sql = "update student set profile_pic = '".$newname."' where entryno='".$_SESSION['entryno']."' ;";
...
function chk_selected($entryno,$cid,$conn){
	
	$sql = "select count(*) from interested natural join student where entryno='".$entryno."' and status='selected' and cid = '".$cid."';";
	//$sql = "select * from interested natural join student where entryno='".$entryno."' and status='selected';";
	$q = $conn->query($sql) or die('failed');
	$r = $q->fetch(PDO::FETCH_ASSOC);
	if($r['count'] != '0'){
		return true;
